...
const Project = () => {
  ...
  return (
    <div id="projects">
      <div className="w-full flex flex-col items-center justify-center px-4 md:px-8 py-16 md:py-24 min-h-[calc(100vh-64px)] mt-10 relative">
        <div className="text-center">
          <p className={`mt-1 text-[14px] w-max mx-auto px-3 mb-4 rounded-full bg-amber-700/30 ${isDark ? 'text-white' : 'text-black'}`}>
            Projects
          </p>
          <h1 className={`text-3xl md:text-5xl font-bold ${isDark ? 'text-orange-600' : 'text-orange-400'}`}>
            Stuffs I Built
          </h1>
        </div>

        <div className="mt-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 max-w-6xl gap-8 w-full">
          {projects.map((project) => (
            <div
              key={project.name}
              className={`flex flex-col py-8 px-8 border border-orange-600/20 rounded-xl shadow-md transition-transform duration-300 hover:scale-[1.03]
                ${isDark ? 'bg-black/40 text-white' : 'bg-white/30 text-black'}`}
            >
              <h2 className="text-xl font-semibold mb-4 text-orange-500">{project.name}</h2>
              <p className="flex-1 mb-6 text-sm leading-relaxed">{project.description}</p>
              <div className="flex gap-2 flex-wrap">
                <Link
                  href={`/projects/${project.slug}`}
                  className="rounded-lg text-orange-100 hover:scale-105 transition-all duration-300 bg-amber-800 px-4 py-1.5 inline-block text-sm font-medium"
                >
                  View Details
                </Link>
                {project.liveUrl && (
                  <a
                    ...
                    className={`rounded-lg hover:scale-105 transition-all duration-300 border border-orange-600/50 px-4 py-1.5 inline-flex items-center gap-1 text-sm font-medium
                      ${isDark ? 'text-orange-300 hover:bg-orange-600/10' : 'text-orange-700 hover:bg-orange-100'}`}
                  >
